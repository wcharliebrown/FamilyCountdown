...
struct FlipMetrics: Equatable {
    ...
    static let fontName = "SplitFlapTVBlackLine-Regular"

    static let digitColor = Color.white                              // tile color, matches the name labels
    static let cardTop = Color(red: 0.10, green: 0.10, blue: 0.10)    // upper leaf
    static let cardBottom = Color(red: 0.06, green: 0.06, blue: 0.06) // lower leaf
    static let seam = Color.black

    // Geometry ratios calibrated to the Split-Flap TV BlackLine font, which is
    // monospaced with a 1.196em advance and a ~1.0em × 1.4em tile per glyph.
    static let advanceRatio: CGFloat = 1.196   // one character cell = 1.196 × fontSize wide
    static let widthRatio: CGFloat = 1.10      // card width: full tile + a little padding
    static let heightRatio: CGFloat = 1.44     // card height: full (tall) flap tile
    static let intraSpacingRatio: CGFloat = 0.08   // gap between digits in a group
...
private struct Leaf: View {
    ...
    var body: some View {
        Text(String(char))
            .font(.custom(FlipMetrics.fontName, fixedSize: metrics.fontSize))
            .foregroundStyle(FlipMetrics.digitColor)
            .frame(width: metrics.width, height: metrics.height)   // glyph centered in full card
            ...
            .clipped()
            .background(half == .top ? FlipMetrics.cardTop : FlipMetrics.cardBottom)
    }
}
